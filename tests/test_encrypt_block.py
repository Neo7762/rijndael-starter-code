...
def test_aes_encrypt_block_known_128(lib):
    plaintext = bytes([0x00] * 16)
    key = bytes([0x00] * 16)
    
    aes_instance = aes.AES(key)
    expected = aes_instance.encrypt_block(plaintext)
    
    buf_plain = (c_ubyte * 16)(*plaintext)
    buf_key = (c_ubyte * 16)(*key)
    encrypted = lib.aes_encrypt_block(buf_plain, buf_key, AES_BLOCK_128)
    result = bytes(cast(encrypted, POINTER(c_ubyte * 16)).contents)
    
    print(f"Expected: {expected.hex()}")
    print(f"Got:      {result.hex()}")
    assert result == expected

def test_aes_encrypt_block_random_128(lib):
    for _ in range(3):
        ...
        assert result == expected, f"Failed for plaintext: {plaintext.hex()}, key: {key.hex()}"

def test_aes_encrypt_block_known_256(lib):
    ...
    # Round-trip test: decrypt should recover plaintext
...
def test_aes_encrypt_block_random_256(lib):
    for _ in range(3):
        ...
        assert result == plaintext, f"Round-trip failed for 256-bit block"

def test_aes_encrypt_block_known_512(lib):
...
def test_aes_encrypt_block_random_512(lib):
    for _ in range(3):
        ...
        assert result == plaintext, f"Round-trip failed for 512-bit block"