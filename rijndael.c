 ...
 * This code implements the Rijndael, AES, encryption and decryption cipher for
 * 128-bit, 256-bit, and 512-bit block sizes. It includes the necessary
 * functions for the transformations that are used in Rijndael, including
...
static void rotate_column_up(unsigned char *block, size_t row_len, size_t col,
                             size_t shift) {
...
static void rotate_column_down(unsigned char *block, size_t row_len, size_t col,
                               size_t shift) {
  unsigned char temp[16];  // max 4 rows for 128-bit
